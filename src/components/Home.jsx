...
export default function Home() {
  ...
  const getNewQuestion = async (category) => {
    try {
      const response = await fetch(`http://localhost:5000/get-question?category=${category}`)
      const data = await response.json()
    ...
    <main className="min-h-screen bg-gradient-to-b from-pink-50 to-white">
      <div className="container mx-auto px-4 py-8">
        <header className="bg-pink-700 text-white rounded-xl p-6 shadow-lg mb-8">
          ...
          <p className="text-center text-pink-200 mt-1">¿Listos para jugar? ¡vamos!</p>
        </header>

        {gameState === "start" && <StartScreen onStartGame={handleStartGame} />}

        {gameState === "category" && <CategorySelection onSelectCategory={handleSelectCategory} />}
        ...
        {gameState === "results" && (
          <ResultsScreen answeredQuestions={answeredQuestions} userName={userName} onPlayAgain={resetGame} />
        )}
      </div>
    </main>
  )
